...
impl HtmlGenerator {
    ...
    fn generate_header(&self) -> String {
        ...
        header.push_str(&format!("<span class=\"version\">v{}</span>", self.config.version));
        header.push_str("</div>");
    ...
    fn generate_operation_documentation(&self, method: &str, operation: &Value, path: &str) -> String {
        ...
        if let Some(params_array) = parameters.as_array() {
            if !params_array.is_empty() {
                ...
                for param in params_array {
                    ...
                    let description = param.get("description").and_then(|d| d.as_str()).unwrap_or("");
    ...
    fn generate_request_body_documentation(&self, request_body: &Value) -> String {
        ...
        let required = request_body.get("required").and_then(|r| r.as_bool()).unwrap_or(false);
        if required {
            ...
            if let Some(content_obj) = content.as_object() {
                for (media_type, media_obj) in content_obj {
                    ...
                    if let Some(schema) = media_obj.get("schema") {
                        ...
                        body_doc.push_str(&serde_json::to_string_pretty(schema).unwrap_or_default());
                    ...
                    if let Some(content_obj) = content.as_object() {
                        for (media_type, media_obj) in content_obj {
                            ...
                            if let Some(schema) = media_obj.get("schema") {
                                ...
                                responses_doc.push_str(&serde_json::to_string_pretty(schema).unwrap_or_default());
            ...
            if let Some(params_array) = parameters.as_array() {
                for param in params_array {
                    ...
                    try_it.push_str(&format!(
                        ...
                        name, name, param_in, if required { " *" } else { "" }
                    ));
                    
                    try_it.push_str(&format!(
                        "<input type=\"text\" id=\"{}\" name=\"{}\" data-in=\"{}\" {}>",
                        name, name, param_in, if required { "required" } else { "" }
                    ));
    ...
    fn generate_footer(&self) -> String {
        ...
        footer.push_str("<p>Generated by DevDocs Pro - Real-time API documentation from traffic analysis</p>");
        
        if let Some(contact) = &self.config.contact {
            if let Some(email) = &contact.email {
                footer.push_str(&format!("<p>Contact: <a href=\"mailto:{}\">{}</a></p>", email, email));
            }
        }
