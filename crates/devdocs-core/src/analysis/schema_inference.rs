...
impl SchemaInferrer {
    ...
    pub async fn infer_from_json_bodies(&self, bodies: &[&CapturedBody]) -> Result<JsonSchema> {
        ...
        if samples_processed == 0 {
            return Err(DevDocsError::InvalidRequest(
                "No valid JSON samples found".into(),
            ));
        }
    ...
    pub async fn extract_json_examples(
        &self,
        bodies: &[&CapturedBody],
        limit: usize,
    ) -> Vec<String> {
...
impl JsonSchemaBuilder {
    ...
    fn build(&self, min_samples_for_required: usize) -> JsonSchema {
        ...
        let top_level_props: HashMap<_, _> = self
            .property_types
            .iter()
    ...
    fn detect_enum(examples: &[Value]) -> Option<Vec<String>> {
        ...
        let string_values: Vec<_> = examples.iter().filter_map(|v| v.as_str()).collect();
