...
impl RateLimiter {
    ...
    pub async fn check_rate_limit(&self, ip: &str) -> Result<(), DevDocsError> {
        ...
        if self.is_ip_blacklisted(ip) {
            return Err(DevDocsError::RateLimit("IP address is blacklisted".to_string()));
        }
        ...
        // Check global rate limit
        {
            ...
            if !global_bucket.try_consume(1.0) {
                return Err(DevDocsError::RateLimit("Global rate limit exceeded".to_string()));
            }
        }
    ...
    pub async fn check_user_rate_limit(&self, user_id: &str) -> Result<(), DevDocsError> {
        ...
        if !bucket.try_consume(1.0) {
            return Err(DevDocsError::RateLimit("User rate limit exceeded".to_string()));
        }
    ...
    async fn check_ip_rate_limit(&self, ip: &IpAddr) -> Result<(), DevDocsError> {
        ...
        if !bucket.try_consume(1.0) {
            return Err(DevDocsError::RateLimit("IP rate limit exceeded".to_string()));
        }
    ...
    async fn track_request(&self, ip: &IpAddr) {
        ...
        let cutoff = now - chrono::Duration::seconds(self.config.ddos_protection.detection_window_seconds as i64);
        ...
        if let Some(requests) = request_counts.get(ip) {
            if requests.len() > self.config.ddos_protection.detection_threshold as usize {
                if self.config.ddos_protection.auto_block_suspicious_ips {
                    drop(request_counts);
                    self.block_ip(ip, self.config.ddos_protection.block_duration_seconds).await;
                }
                return Err(DevDocsError::RateLimit("DDoS protection triggered".to_string()));
            }
        }
