...
impl DataEncryptor {
    ...
    pub fn encrypt(&self, data: &[u8], context: &str) -> Result<Vec<u8>, DevDocsError> {
        ...
        if key.is_expired() {
            return Err(DevDocsError::Encryption(
                "Current key has expired".to_string(),
            ));
        }

        match self.config.algorithm {
            EncryptionAlgorithm::Aes256Gcm => self.encrypt_aes_gcm(data, key, context),
            EncryptionAlgorithm::ChaCha20Poly1305 => Err(DevDocsError::Encryption(
                "ChaCha20Poly1305 not implemented yet".to_string(),
            )),
        }
    }
    ...
    pub fn decrypt(&self, encrypted_data: &[u8]) -> Result<Vec<u8>, DevDocsError> {
        ...
        let key = self.keys.get(&encrypted.key_id).ok_or_else(|| {
            DevDocsError::Encryption(format!("Key {} not found", encrypted.key_id))
        })?;
    ...
    pub fn rotate_key(&mut self) -> Result<(), DevDocsError> {
        ...
        let expires_at =
            chrono::Utc::now() + chrono::Duration::hours(self.config.key_rotation_hours as i64 * 2); // Keep old keys for 2x rotation period

        let secure_key = SecureKey::new(key_id.clone(), key_data).with_expiry(expires_at);
    ...
    pub fn get_encryption_metadata(&self) -> EncryptionMetadata {
        EncryptionMetadata {
            ...
            last_rotation: self.keys.get(&self.current_key_id).map(|k| k.created_at),
        }
    }
}
...
mod tests {
    ...
    fn test_secure_key_expiry() {
        ...
        let key = SecureKey::new("test_key".to_string(), key_data).with_expiry(past_time);
