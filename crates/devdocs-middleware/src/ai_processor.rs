...
use devdocs_core::{
    ...
    models::{TrafficSample, AIAnalysisResult, GeneratedExample},
};
...
impl AIProcessorService {
    ...
    pub fn with_documentation_updates(mut self, sender: mpsc::UnboundedSender<DocumentationUpdate>) -> Self {
    ...
    async fn process_batch(&mut self) {
        ...
        tokio::spawn(async move {
            ...
            match analyzer.analyze_endpoint_samples(&samples).await {
                ...
                Err(e) => {
                    tracing::error!("AI analysis failed for batch of {} samples: {}", sample_count, e);
                }
            }
        });
    }
